...
Usage:
    ...
    python scripts/run_sweep.py configs/default.yaml --output-dir results
"""
...
from vfd_cbd.sweep import run_sweep, get_sweep_summary
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a CBD parameter sweep."
    )
    parser.add_argument(
        "config",
        type=Path,
    ...
    if not args.quiet:
        print(f"System: N={config.system.N} oscillators, geometry={config.system.geometry}")
        print(f"Sweep: {config.sweep.n_df} x {config.sweep.n_k} = {total_points} points")
    ...
    # Run sweep
    result = run_sweep(config, progress_callback=progress_callback)
    ...
    if not args.quiet:
        print()
        print("Summary:")
        print(f"  Stable points: {summary['stable_points']}/{summary['total_points']} ({summary['stable_fraction']*100:.1f}%)")
        print(f"  Mean coherence: {summary['C_mean_overall']:.3f}")
        print(f"  Elapsed time: {summary['elapsed_seconds']:.2f}s ({summary['points_per_second']:.1f} points/s)")
